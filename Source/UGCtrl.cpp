...
int CUGCtrl::AddBitmap( LPCTSTR fileName){
	...
	if(m_bmih.biBitCount <16)
		m_lpbmi = (BITMAPINFO *)new char[sizeof(BITMAPINFOHEADER)+(static_cast<size_t>(1<<m_bmih.biBitCount) * sizeof(RGBQUAD))];
	else
	...
	if(m_bmih.biBitCount <16)
		ifs.read((char*) &m_lpbmi->bmiColors, (static_cast<size_t>(1<<m_bmih.biBitCount) * sizeof(RGBQUAD)));
