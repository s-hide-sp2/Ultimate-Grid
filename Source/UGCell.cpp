...
int CUGCell::SetDefaultInfo()
{
	...
	// clear the cell's pointer value
	m_lpPointer = nullptr;
...
int CUGCell::AddCellInfo(CUGCell *src,CUGCell *dest)
{
	...
	if(src->m_propSetFlags&UGCELL_POINTER_SET)
		dest->m_lpPointer = src->m_lpPointer;
...
int CUGCell::SetPointer( LPVOID value )
{
	m_propSetFlags |= UGCELL_POINTER_SET;
	m_lpPointer = value;

	return UG_SUCCESS;
}

LPVOID CUGCell::GetPointer() const
{
	if(m_propSetFlags & UGCELL_POINTER_SET)
		return m_lpPointer;
	return nullptr;
}
