...
LRESULT CUTEdit::OnClear(WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
int CUTEdit::GetNextInputLocation(int nSelectionStart)
{
	...
	{
		nNextInputLocation = static_cast<int>( m_arrLeftLiterals.GetSize()+m_arrInputData.GetSize() );
...
int CUTEdit::GetPreviousInputLocation(int nSelectionStart)
{
	...
	{
		nPreviousInputLocation=static_cast<int>(m_arrLeftLiterals.GetSize());
		for(int nIndex=nSelectionStart; 
...
int CUTEdit::RPtoLP(int nRealPos)const 
{
	...
	if(nRealPos<m_arrLeftLiterals.GetSize() || 
		...
		return nRealPos - static_cast<int>( m_arrLeftLiterals.GetSize() );
}
...
int CUTEdit::LPtoRP(int nLogicalPos)const 
{
	if(nLogicalPos<0 || nLogicalPos>m_arrInputData.GetSize())
		...
		return nLogicalPos + static_cast<int>( m_arrLeftLiterals.GetSize() );
}
...
int CUTNumericEdit::ValidateInput()
{
	...
	if(GetGroupLength()>0)
	{
		...
		nDecimalDelimiterPos=(nDecimalDelimiterPos==-1) ?
			static_cast<int>(m_arrInputData.GetSize()) : nDecimalDelimiterPos;
	...
	int nSymbolsAfter = static_cast<int>( m_arrInputData.GetSize() );
...
int CUTNumericEdit::ChangeSign(BOOL bNegative)
{
	...
	int nLeftLiteralsBefore = static_cast<int>( m_arrLeftLiterals.GetSize() );
	...
	return static_cast<int>(m_arrLeftLiterals.GetSize()) - nLeftLiteralsBefore;
}
...
BOOL CUTNumericEdit::SetValue(double dValue)
{
	...
	size_t nSymbolCount=strlen(buffer);
									...
									TCHAR chNewChar, BOOL bReplaceExisting)
{
		...
		(m_bForceToUsePeriod && chNewChar==_T('.')))
	{
			...
			(m_bForceToUsePeriod && chNewChar==_T('.')))
		{
			...
			if(nPos==-1)
			{
				nSelectionStart = static_cast<int>(m_arrLeftLiterals.GetSize()+m_arrInputData.GetSize());
...
int CUTNumericEdit::GetDecimalDigitsInserted() const
{
	...
	int nDecimalDelimiterPos=FindSymbol(GetDecimalSeparator());
	nDecimalDelimiterPos=(nDecimalDelimiterPos==-1 ? 
		static_cast<int>(m_arrInputData.GetSize()) : RPtoLP(nDecimalDelimiterPos));
